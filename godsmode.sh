...
\f0\fs26 \cf0 #!/bin/sh\
# ============================================================\
# Flint 2 GOD MODE \'97 Kernel 5.4.238 SAFE\
...
\
    ...
    if [ "$new_state" != "$prev_state" ]; then\
      ...
      send_telegram "[LIVE] $\{new_state\} SQM=$\{target_down\}/$\{target_up\} cake=$\{cake_conf\} drops\uc0\u916 =$\{dd\} backlog=$\{backlog\}B fps=$\{fps\} ft=$\{ft\}ms"\
      echo "[LIVE] $\{new_state\} SQM=$\{target_down\}/$\{target_up\} cake=$\{cake_conf\} drops\uc0\u916 =$\{dd\} backlog=$\{backlog\}B fps=$\{fps\} ft=$\{ft\}ms"\
    fi\
\
